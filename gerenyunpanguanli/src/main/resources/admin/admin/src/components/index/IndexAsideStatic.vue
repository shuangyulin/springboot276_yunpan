<template>
  <el-aside class="index-aside" height="100vh" width="280px">
    <div class="index-aside-inner menulist" style="height:100%">
      <div v-for="item in menuList" :key="item.roleName" v-if="role==item.roleName" class="menulist-item" style="height:100%;broder:0;background-color:#202020">
        <div class="menulistImg" style="backgroundColor:#202020;padding:30px 0" v-if="false && menulistStyle == 'vertical'">
        ...
        <el-menu mode="vertical" :unique-opened="true" class="el-menu-demo" style="height:100%;" background-color="#202020" text-color="#ffffff" active-text-color="#FFFFFF" default-active="0">
      ...
      this.$nextTick(()=>{
        document.querySelectorAll('.menulist .el-menu-item').forEach(el=>{
          el.addEventListener("mouseenter", e => {
            e.stopPropagation()
            el.style.backgroundColor = "rgba(201, 0, 0, 1)"
          })
